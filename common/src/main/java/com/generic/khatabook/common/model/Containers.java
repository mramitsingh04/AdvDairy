...
public class Containers<T, U> implements Iterable<Container<T, U>> {
    ...
    public Iterator<T> iteratorKey() {
        return myList.stream().map(Container::get).toList().iterator();
    }

    public Iterator<U> iteratorUpdatable() {
        return myList.stream().map(Container::updatable).toList().iterator();
    }
